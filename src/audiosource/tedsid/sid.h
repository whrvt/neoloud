...
public:
	...
	virtual ~SIDsound();
	void reset();
	void setReplayFreq() { calcEnvelopeTable(); };
	...
	void enableDisableChannel(unsigned int ch, bool enabled) { voice[ch].disabled = !enabled; }
...
{
	unsigned int msb = (v.ring ? v.accu ^ v.modulatedBy->accu : v.accu) & 0x8000000;
