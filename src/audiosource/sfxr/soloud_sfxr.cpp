...
unsigned int SfxrInstance::getAudio(float *aBuffer, unsigned int aSamplesToRead, unsigned int /*aBufferSize*/)
{
	...
	for (i = 0; i < aSamplesToRead; i++)
	{
		...
		{
			vib_phase += vib_speed;
			rfperiod = (float)(fperiod * (1.0 + std::sin(vib_phase) * vib_amp));
		}
		period = (int)rfperiod;
		if (period < 8)
			period = 8;
		...
		if (env_stage == 1)
		{
			if (env_length[1])
			{
				env_vol = 1.0f + (float)std::pow(1.0f - (float)env_time / env_length[1], 1.0f) * 2.0f * mParams.p_env_punch;
			}
			else
			{
				env_vol = 0;
...
void SfxrInstance::resetSample(bool aRestart)
{
	...
	arp_time = 0;
	arp_limit = (int)(std::pow(1.0f - mParams.p_arp_speed, 2.0f) * 20000 + 32);
	...
	if (!aRestart)
	{
		...
		fltp = 0.0f;
		fltdp = 0.0f;
		fltw = (float)std::pow(mParams.p_lpf_freq, 3.0f) * 0.1f;
		fltw_d = 1.0f + mParams.p_lpf_ramp * 0.0001f;
		fltdmp = 5.0f / (1.0f + (float)std::pow(mParams.p_lpf_resonance, 2.0f) * 20.0f) * (0.01f + fltw);
		if (fltdmp > 0.8f)
			fltdmp = 0.8f;
		fltphp = 0.0f;
		flthp = (float)std::pow(mParams.p_hpf_freq, 2.0f) * 0.1f;
		flthp_d = (float)(1.0 + mParams.p_hpf_ramp * 0.0003f);
		// reset vibrato
		vib_phase = 0.0f;
		vib_speed = (float)std::pow(mParams.p_vib_speed, 2.0f) * 0.01f;
		...
		fphase = (float)std::pow(mParams.p_pha_offset, 2.0f) * 1020.0f;
		if (mParams.p_pha_offset < 0.0f)
			fphase = -fphase;
		fdphase = (float)std::pow(mParams.p_pha_ramp, 2.0f) * 1.0f;
		...
		rep_time = 0;
		rep_limit = (int)(std::pow(1.0f - mParams.p_repeat_speed, 2.0f) * 20000 + 32);
